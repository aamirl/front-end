sourceAdminApp.controller('roomsCtrl', ['$rootScope', '$scope', '$http', '$timeout', '$cookies', '$location', 'api', 'chatWSService', '$compile', 'apiCall', 'Alertify', 'storageService', function($rootScope, $scope, $http, $timeout, $cookies, $location, api, chatWSService, $compile, apiCall, Alertify, storageService){
   $scope.rooms = [];
   chatWSService.close();
   ...
   var req = {
       method : "GET",
       url : "https://chatting.sellyx.com/room/list?longitude="+$scope.user_location.longitude+"&latitude="+$scope.user_location.latitude+"&radius=1000",
       headers : {
...
function initialize() {
  ...
  var mapOptions = {
      ...
      minZoom: 2,
      maxZoom:13,
      center: new google.maps.LatLng($scope.user_location.latitude, $scope.user_location.longitude),
